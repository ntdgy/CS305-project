    ...
    def recvfrom(self, bufsize, flags=0):
        ...
        if ret is not None:
            ...
            (
                ...
                s_head_lSrcPort,
                s_head_lDestPort,
            ) = struct.unpack("I4s4sHH", simu_bytes[: self.__spiffyHeaderLen])
            from_addr = (
                socket.inet_ntoa(s_head_lSrcAddr),
                socket.ntohs(s_head_lSrcPort),
            )
            to_addr = (
                socket.inet_ntoa(s_head_lDestAddr),
                socket.ntohs(s_head_lDestPort),
            )
